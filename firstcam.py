...
def start_camera_stream():
    camera_client = CameraClient(text.get(1.0, 'end-1c'), 7777)
    t3 = threading.Thread(target=camera_client.start_stream)
    t3.start() ###feito

def start_screen_sharing():
    screen_sharing_client = ScreenShareClient(text.get(1.0, 'end-1c'), 7777)
    t4 = threading.Thread(target=screen_sharing_client.start_stream)
    t4.start() ###feito

def start_audio_stream():
    audio_sender = AudioSender(text.get(1.0, 'end-1c'), 6666)
